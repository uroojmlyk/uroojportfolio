...
export default function Hero() {
  ...
  return (
    <section className="relative w-full min-h-screen bg-[#E5E5E5] overflow-hidden">
      ...
      {/* CONTENT */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full h-screen flex items-center">
        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-0 items-center w-full h-full">
          
          {/* LEFT SIDE - UROOJ TEXT */}
          <div ref={textRef} className="relative w-full h-full flex items-center justify-center lg:justify-start">
            <div className="relative w-full flex items-center justify-center lg:justify-start">
              {/* MAIN TITLE - MOBILE PE BILKUL CENTER, POORA UROOJ */}
              <h1 className="text-[120px] xs:text-[140px] sm:text-[160px] md:text-[180px] lg:text-[280px] xl:text-[320px] 2xl:text-[360px] font-black leading-[0.7] text-[#FF8A5C]/90 tracking-[-0.05em] select-none text-center lg:text-left animate-fadeIn relative z-0 whitespace-nowrap">
                UROOJ
              </h1>
              
              {/* GLOW EFFECT */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] sm:w-[500px] md:w-[600px] lg:w-[700px] h-[150px] sm:h-[180px] md:h-[200px] lg:h-[250px] bg-[#FF8A5C]/20 blur-[100px] rounded-full -z-10 animate-pulse-slow"></div>
              
              {/* SUBTITLE - MOBILE PE CENTER */}
              <div className="absolute -bottom-16 sm:-bottom-20 left-1/2 -translate-x-1/2 lg:left-1 lg:-translate-x-0 animate-slideUp z-30 w-full px-4">
                <span className="block text-[#333333] text-xs sm:text-sm md:text-base lg:text-lg font-semibold tracking-[0.25em] sm:tracking-[0.3em] uppercase text-center lg:text-left">
                  FULL STACK DEVELOPER
                </span>
                <span className="block text-[#666666] text-[10px] sm:text-xs md:text-sm lg:text-base font-medium tracking-[0.2em] sm:tracking-[0.25em] uppercase mt-1 sm:mt-2 text-center lg:text-left">
                  & WEB DESIGNER
                </span>
              </div>
            </div>
          ...
          {/* RIGHT SIDE - IMAGE (HAMESHA DIKHEGI) */}
          <div ref={imageRef} className="relative w-full h-full flex items-center justify-center lg:justify-end z-20">
            <div className="relative w-[250px] xs:w-[280px] sm:w-[320px] md:w-[380px] lg:w-[500px] xl:w-[550px] 2xl:w-[600px] h-[320px] xs:h-[350px] sm:h-[400px] md:h-[450px] lg:h-[600px] xl:h-[650px] 2xl:h-[700px] animate-float">
              {/* IMAGE CONTAINER */}
              <div className="absolute inset-0 flex items-center justify-center z-30">
                <Image 
                  src="/freen2bg.png" 
              ...
              {/* DECORATIVE DOTS - MOBILE PE CHHOTE */}
              <div className="absolute -top-3 -right-3 lg:-top-5 lg:-right-5 w-2 h-2 lg:w-3 lg:h-3 bg-[#FF8A5C] rounded-full animate-ping z-40"></div>
              <div className="absolute -bottom-3 -left-3 lg:-bottom-5 lg:-left-5 w-1.5 h-1.5 lg:w-2.5 lg:h-2.5 bg-[#A0A0A0] rounded-full animate-pulse z-40"></div>
              <div className="absolute top-1/3 -right-4 lg:-right-7 w-2 h-2 lg:w-3 lg:h-3 border-2 border-[#FF8A5C]/50 rounded-full animate-spin-slow z-40"></div>
              <div className="absolute bottom-1/3 -left-4 lg:-left-6 w-1.5 h-1.5 lg:w-2 lg:h-2 bg-[#FF8A5C]/40 rounded-full animate-bounce z-40"></div>
              
              {/* DECORATIVE LINES - MOBILE PE CHHOTE */}
              <div className="absolute top-1/2 -right-6 lg:-right-10 w-0.5 lg:w-1 h-16 lg:h-24 bg-gradient-to-b from-[#FF8A5C]/30 to-transparent rotate-45 animate-pulse-slow"></div>
              <div className="absolute bottom-10 -left-6 lg:-left-10 w-0.5 lg:w-1 h-12 lg:h-20 bg-gradient-to-t from-[#FF8A5C]/30 to-transparent -rotate-12 animate-pulse-slow"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* BOTTOM NAVIGATION */}
      <div className="absolute bottom-4 sm:bottom-8 left-1/2 -translate-x-1/2 w-full max-w-6xl px-4 sm:px-6 z-50 animate-fadeInUp">
        <div className="flex flex-wrap justify-center items-center gap-3 sm:gap-4 lg:gap-6 text-[8px] xs:text-[9px] sm:text-[10px] md:text-xs font-medium tracking-[0.2em] sm:tracking-[0.25em] lg:tracking-[0.3em] text-[#666666] border-t border-[#CCCCCC]/50 pt-4 sm:pt-6 uppercase">
          {['HOME', 'SERVICES', 'PROJECTS', 'EXPERIENCES', 'CASE STUDY', 'CONTACT'].map((item) => (
            <span 
              key={item}
              className="relative hover:text-[#FF8A5C] transition-colors duration-300 cursor-pointer whitespace-nowrap group"
      ...
      <style jsx>{`
        ...
        @media (min-width: 320px) {
          .xs\\:text-[140px] { font-size: 140px; }
          .xs\\:text-[9px] { font-size: 9px; }
          .xs\\:w-[280px] { width: 280px; }
